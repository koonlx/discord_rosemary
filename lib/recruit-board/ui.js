...
const buildRecruitModal = (
   ...
   if (enableHostInput) {
      const hostInput = new TextInputBuilder()
         .setCustomId(HOST_DISCORD_INPUT_ID)
         .setLabel('모집자 / 신청자 (선택)')
         .setPlaceholder('host: @모집자\nmembers: @신청자1, @신청자2')
         .setRequired(false)
         .setMaxLength(400)
         .setStyle(TextInputStyle.Paragraph);

      if (hostInputValue) {
         hostInput.setValue(hostInputValue);
      }

      components.push(new ActionRowBuilder().addComponents(hostInput));
   }
